// src/scene/SarcophagusChamberScene.tsx
...
export default function SarcophagusChamberScene({ onProceed }: { onProceed: () => void }) {
  const { doors, progress } = useRoom();
  const [facing, setFacing] = useState<Facing>("front");
  const [panel, setPanel] = useState<Panel>("none");

  const gateOpen = doors.find(d => d.key === "sarcophagus_gate")?.state === "open";
  const solved = progress.some(p => p.puzzle_key === "canopic" && p.solved);
  const canProceed = gateOpen || solved;
  ...
  const caption = useMemo(() => {
    if (facing === "front") return gateOpen ? "Sarcophagus Gate — OPEN" : "Sarcophagus Gate";
    if (facing === "left") return "Jar Wall (Indices)";
    return "Mummification Panel (Indices)";
  }, [facing, gateOpen]);
  ...
  return (
    <div className="relative min-h-[calc(100vh-2rem)] max-w-6xl mx-auto my-4 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
      ...
      <SceneBG facing={facing} gateOpen={gateOpen} />
      ...
      <div className="absolute inset-x-0 bottom-6 z-10 flex items-center justify-center gap-3">
        ...
        {!canProceed ? (
          ...
          >
            {facing === "front" ? "Inspect Gate (puzzle)" : "Read Wall (indices)"}
          </button>
        ) : (
          <button
            onClick={onProceed}
...
function SceneBG({ facing, gateOpen }: { facing: Facing; gateOpen: boolean }) {
        ...
        >
          {!gateOpen ? (
            <div className="absolute inset-8 flex items-center justify-center">
              <div className="w-2/3 h-1 rounded bg-amber-400/50" />
            </div>
          ) : (
            <div className="absolute -inset-6 rounded-[22px] bg-emerald-400/10 blur-2xl pointer-events-none" />
          )}
          <div className="absolute right-6 top-1/3 w-10 h-10 rounded-full border-2 border-amber-500/70" />
        </div>
      </div>
    </div>
  );
        ...
        <div className="rounded-xl border border-amber-900/30 p-4 md:p-5 bg-[radial-gradient(ellipse_at_top,_#fff7e6,_#f1e0bf)] text-zinc-900 shadow-inner">
          <div className="space-y-3 text-sm leading-relaxed">
            <p className="text-zinc-800">
              Canopic jars stored organs removed during <strong>mummification</strong>. Each jar has a deity guardian:
            </p>
            <ul className="list-disc pl-5 text-sm text-zinc-800/90">
              <li><strong>Imsety</strong> (human head) — <em>liver</em></li>
              <li><strong>Hapi</strong> (baboon) — <em>lungs</em></li>
              <li><strong>Duamutef</strong> (jackal) — <em>stomach</em></li>
              <li><strong>Qebehsenuef</strong> (falcon) — <em>intestines</em></li>
            </ul>
            <p className="text-amber-900/80"><strong>Tip:</strong> Match deity to organ, not just the animal head.</p>
          </div>
        </div>
