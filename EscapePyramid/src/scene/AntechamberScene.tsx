...
export default function AntechamberScene({ onProceed }: { onProceed: () => void }) {
  const { roomId, roomCode, doors } = useRoom();
  const { progress } = useRoom();
  const cartoucheSolved = progress.some(p => p.puzzle_key === 'cartouche' && p.solved);
